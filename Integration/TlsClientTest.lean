...
Lean gRPC server → refinement-type validation/authz → pg-lean → postgres —
plus graceful listener termination.
...
def main : IO Unit := do
  ...
  let server ← Grpc.Server.serveTls (Acme.Service.registry repo)
  ...
  -- editor creating its own widget: authorized, persisted
  ...
  match ← Async.block (Grpc.Client.call client path denyBytes) with
  ...
  -- cross-principal denial (authz.create.self)
  let crossBytes ← encode! (checkedCreate { id := 8, role_level := 2 } 7).encode
  match ← Async.block (Grpc.Client.call client path crossBytes) with
